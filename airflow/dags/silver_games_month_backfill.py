...
def _materialize_month_from_conf(ds: str, **_kwargs) -> None:
    ...
    month_keys = silver_service.resolve_month_keys_from_conf(conf, ds)
    silver_service.materialize_selected_months(month_keys)


with DAG(
    dag_id="silver_games_month_backfill",
    description="Materialize selected historical months or years into silver parquet datasets",
    ...
    tags=["chess", "silver", "games", "backfill"],
    max_active_runs=1,
    max_active_tasks=1,
) as dag:
    PythonOperator(
        task_id="materialize_month_silver",
        python_callable=_materialize_month_from_conf,
        op_kwargs={
            "ds": "{{ ds }}",
        },
    )
