...
def _build_gold_from_conf(ds: str, **_kwargs) -> None:
    import dbt_runner
    ...
    dbt_runner.build_gold_selected(month_keys=month_keys)


with DAG(
    dag_id="dbt_gold_backfill",
    description="Build selected historical gold marts in dbt from the silver medallion layer",
    ...
    tags=["chess", "dbt", "gold", "backfill"],
    max_active_runs=1,
    max_active_tasks=1,
) as dag:
    PythonOperator(
        task_id="build_gold_models",
        python_callable=_build_gold_from_conf,
        op_kwargs={
            "ds": "{{ ds }}",
        },
    )
