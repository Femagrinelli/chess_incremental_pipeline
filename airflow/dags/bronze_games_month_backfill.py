...
def _materialize_months_from_conf(ds: str, **_kwargs) -> None:
    ...
    month_keys = bronze_service.resolve_month_keys_from_conf(conf, ds)
    bronze_service.materialize_selected_months(month_keys)


with DAG(
    dag_id="bronze_games_month_backfill",
    description="Materialize selected months into canonical bronze game parquet datasets",
    ...
    tags=["chess", "bronze", "games", "backfill", "parquet"],
    max_active_runs=1,
    max_active_tasks=1,
) as dag:
    PythonOperator(
        task_id="materialize_bronze_months",
